...
const props = defineProps<{ kind: PaneKind; universe: Universe; theme: Theme; revision: number; label: string; quiet: boolean; paused: boolean }>();
...
function draw(force = false) {
  ...
  if (moving()) { clock += dt; blendVisual(visual, props.universe, 1 - Math.exp(-dt * 7)); }
...
watch(() => [props.quiet, props.paused], () => { visual = structuredClone(props.universe); sync(); });
onMounted(() => {
